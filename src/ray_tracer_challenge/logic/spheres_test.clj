...
(deftest about-spheres
  ...
  (testing "should be able to calculate normal"
    (let [sphere (sphere)
          a (/ (sqrt 3) 3)]
      (are [x p] (roughly x ((:local-normal-at sphere) sphere p))
  ...
  (testing "should be able to calculate normal on a translated sphere"
    (let [sphere (sphere :transform (translation 0 1 0))]
      (is (roughly (vektor 0 0.70711 -0.70711) ((:local-normal-at sphere) sphere (point 0 0.70711 -0.70711)))))
    (let [sphere (sphere :transform (multiply-matrices (scaling 1 0.5 1) (rotation-z (/ Math/PI 5))))]
      (is (roughly (vektor 0 0.70711 -0.70711) ((:local-normal-at sphere) sphere (point 0 (/ (sqrt 2) 2) (- (/ (sqrt 2) 2))))))))

  (testing "should be able to calculate intersection"
    (let [sphere (sphere)]
      (are [xs origin] (roughly (mapv #(intersection % sphere) xs) ((:local-intersect sphere) sphere (ray origin (vektor 0 0 1))))
                       [4.0 6.0] (point 0 0 -5)
                       [5.0 5.0] (point 0 1 -5)
                       [] (point 0 2 -5)
                       [-1.0 1.0] (point 0 0 0)
  ...
  (testing "should be able to calculate intersection on a scaled sphere"
    (let [local-ray (ray (point 0 0 -2.5) (vektor 0 0 0.5))
          ...
          xs ((:local-intersect sphere) sphere local-ray)]
